...
import 'package:flutter_app/routes/category_listing/index.dart';
...
abstract class _CMSStore with Store {
  ...
  _CMSStore() {
    ...
    _load('categoryListing').then((map) {
      List<CategoryListingPage> pages =
          map.map(CategoryListingPage.fromJSON).toList();
      for (var page in pages) {
        categoryListing[page.id] = page;
      }
    });
  }
  ...
  Future<void> navigate(BuildContext context, CMSLink link) async {
    ...
    if (link.type == CMSPageType.categoryListing) {
      var cms = getCategoryListingPage(link.id);
      Navigator.push(
          context,
          MaterialPageRoute(
              builder: (context) => CategoryListingRoute(cms: cms)));
    }
  }
}
